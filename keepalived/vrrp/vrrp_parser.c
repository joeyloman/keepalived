...
static void
vrrp_failover_delay_handler(vector_t *strvec)
{
	vrrp_t *vrrp = LIST_TAIL_DATA(vrrp_data->vrrp);
	vrrp->failover_delay = atoi(vector_slot(strvec, 1));
	if (VRRP_IS_BAD_FAILOVER_DELAY(vrrp->failover_delay)) {
		log_message(LOG_INFO, "VRRP Error : failover delay not valid !");
		log_message(LOG_INFO,
			"             must be between 0 & 60. reconfigure !");
		log_message(LOG_INFO,
			"             Using default value : %d\n", VRRP_FAILOVER_DELAY_DFL);
		vrrp->failover_delay = VRRP_FAILOVER_DELAY_DFL;
	}
}
...
vrrp_init_keywords(void)
{
	...
	install_keyword("failover_delay", &vrrp_failover_delay_handler);
